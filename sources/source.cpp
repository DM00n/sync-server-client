...
void Server::req_analysis(std::shared_ptr<Client> &b) {
    ...
    if (msg.find(REQ_LOGIN) == 0) login_ok(msg, b);
        ...
        b->get_sock().write_some(boost::asio::buffer(MSG_BAD_MESSAGE));
...
Client::Client(Context *io) : last_ping(0), sock_(*io), _username(NO_NAME) {}
...
void Client::ans_analysis() {
    ...
    else if (msg.find(MSG_TIMED_OUT) == 0)
    {
         working = false;
    }
}
